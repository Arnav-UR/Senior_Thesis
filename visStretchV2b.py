...
for file in files:
    # if str(file).endswith("disk.dat") or str(file).endswith("diss.dat"):
    #     print("oh no")
    #     continue
    with open(file) as r:
        lines = r.readlines()[2:]
...
#   Perform COM-crawl to get the "useful" particles that are actually stretched    
    ...
    # stretchedPart_x = outPart_x
    # stretchedPart_y = outPart_y
    ...
    max_length = (str(round(rawLength[0], sigfigs=4))+' +/- '+str(round(rawLength[1], sigfigs=3))+' m')
    rlvnt_length = str(round((len(accPart)/len(outPart))*rawLength[0], sigfigs=4))
    lbl = "max length: "+max_length+", relevant length: "+rlvnt_length
    plt.plot(xdata, func(xdata, *popt), label=lbl, c='red')
    plt.legend(loc='best')
    print(lbl)
    ...
    if (not os.path.exists('./Time_Evolution_Figs/'+directory+'SLc')):
        print("yeet")
        os.mkdir('./Time_Evolution_Figs/'+directory+'SLc')

    plt.savefig('./Time_Evolution_Figs/'+directory+'SLc/'+str(file)[-9:-4]+'.png')
